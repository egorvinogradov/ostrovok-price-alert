...
function init(callback){
    var bookingRooms = getRoomData(roomConfig);
                    ...
                    getOstrovokRooms({
                        ...
                        success: function(data){
                            if ( data.rooms.length ) {
                                render(bookingRooms, data.rooms);
                                callback && callback(bookingRooms, data.rooms); // TODO: remove
                            }
                        },
                        error: function(e){
                            console.error(e);
...
zcallback = function(bookingRooms, ostrovokRooms){
    console.log('___ ROOMS', ostrovokRooms);
    window.ostrovokRooms = ostrovokRooms;
    window.bookingRooms = bookingRooms;
}

getNames = function(rooms){
    return $.map(rooms, function(room){
        return room.name;
    });
}
...
var soundex = {
    ...
    word: function (str, iscyr) {
        ...
        var output = output.substr(0, power)
            .replace(/^03(8|$)/, '53$1'); // отель, готель, хотел
        if (output && output.length < 3) output += (new Array(3 - output.length + 1))
            .join('0');
